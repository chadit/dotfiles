...
function M.create_mappings(mappings, opts, buffer)
  ...
  local function keymap(...)
    if buffer then
      api.nvim_buf_set_keymap(buffer, ...)
    else
      api.nvim_set_keymap(...)
    end
  end
    ...
    for k, v in pairs(maps) do
      ...
      elseif type(v) == 'table' then
        if #v == 2 then
          ...
          if has_notify then notify(table.concat([[Mapping not run for lhs: ]] .. k .. [[ mode: ]] .. mode, "\n"), "error", {title = [[Mapping]], timeout = 2000}) end
        end
      end
    end
  end
...
--         local syn_group_name = api.nvim_call_function("synIDattr",
--                                                       {syn_group_id, "name"})
